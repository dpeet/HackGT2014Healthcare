...
Template.hospitalsListPage.helpers({
  majorEventsList: function(){
    var majorEventsCount = Hospital.find({$or:[
      {title: { $regex: Session.get('hospitalsSearchFilter'), $options: 'i' }},
      {date: { $regex: Session.get('hospitalsSearchFilter'), $options: 'i' }},
      {time: { $regex: Session.get('hospitalsSearchFilter'), $options: 'i' }},
